...
class ConnectionManager:
                    ...
                    try:
                        ...
                        if target_client_id and message_content:
                            await self.send_personal_message(
                                message_content, target_client_id
                            )
                        else:
                            await self.broadcast(message_content)
                            print(
                                f"Mensagem Redis sem client_id ou content: {msg_data}"
                            )
    ...
    async def connect(self, websocket: WebSocket, client_id: str):
        ...
        print(
            f"Cliente {client_id} conectado. Total de conexões: {len(self.active_connections)}"
        )
        await self.send_personal_message(
            {"type": "status", "message": "Conectado ao Session Manager."}, client_id
        )
    ...
    async def disconnect(self, websocket: WebSocket, client_id: str):
        if client_id in self.active_connections:
            ...
            print(
                f"Cliente {client_id} desconectado. Total de conexões: {len(self.active_connections)}"
            )
    ...
    async def send_personal_message(
        self, message: Union[str, Dict[str, Any]], client_id: str
    ):
        if client_id in self.active_connections:
