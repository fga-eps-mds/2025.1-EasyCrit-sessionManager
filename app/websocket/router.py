...
async def websocket_endpoint(websocket: WebSocket, session_id: str, user_id: int):
  ...
  try:
    ...
    await manager.send_personal_message({"type": "status", "message": f"Conectado à sessão {session_id} como usuário {user_id}."}, client_id)
    ...
    while True:
      ...
      try:
        ...
        logger.error(f'Erro ao processar mensagem recebida de user {user_id} na sessão {session_id}: {e}', exc_info=True)
